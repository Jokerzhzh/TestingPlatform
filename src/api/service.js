...
export const JavaBaseUrl = process.env.VUE_APP_URL_JAVA; // 引入全局url，定义在全局变量process.env中
...
const api = {
    get(url, data, type) {
        return instance.get(type ? JavaBaseUrl + url : baseUrl + url, { params: data });
    },
    post(url, data, type) {
        return instance.post(type ? JavaBaseUrl + url : baseUrl + url, data);
    },
    delete(url, data, type) {
        return instance.delete(type ? JavaBaseUrl + url : baseUrl + url, { data });
    },
    put(url, data, type) {
        return instance.put(type ? JavaBaseUrl + url : baseUrl + url, data);
    }
};
export { api };
