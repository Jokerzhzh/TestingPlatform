    ...
    <div>
        <div class="container">
            <div class="handle-box">
                <el-input style="margin-right:20px" v-model="bproject_name" placeholder="请输入项目名称"></el-input>
            ...
            <el-table :data="tableData" border size="mini" class="table" ref="multipleTable">
                ...
                <el-table-column prop="bid" label="项目ID" width="66" align="center">
                    <template slot-scope="scope">{{ scope.row.bid }}</template>
                </el-table-column>
                <el-table-column prop="bproject_name" label="项目名称" width="150"></el-table-column>
                ...
                <el-table-column label="项目状态" align="center">
                    <template slot-scope="scope">
                        <el-tag
                            :type="scope.row.status == 1 ? 'success' : scope.row.status == 2 ? '' : 'info'"
                        >
                            {{
                ...
                <el-table-column label="负责人" width="200">
                    <template slot-scope="scope">
                        <el-tag
                            ...
                            v-for="(item, index) in scope.row.tester"
                            :key="index"
                ...
                <el-card class="box-card">
                    <div class="box-line">
                        <input
                            type="text"
                            placeholder="请输入名称"
                            v-model="dialogObj.bproject_name"
                            class="project-input"
                        />
                    </div>
                    <textarea v-model="dialogObj.bporject_desc" class="textarea-input" />
                </el-card>
...
export default {
    ...
    methods: {
        ...
        checkDetails(data) {
            this.$api
                .need_project({ belong_project: data.row.bid })
                .then(res => {
        ...
        onSubmit() {
            ...
            if (this.title == '创建项目') {
                ...
                this.$api
                    .modify_big_project(this.dialogObj)
                    .then(res => {
                        if (res.code == 0) {
                            this.$message({
                                message: '编辑成功',
        ...
        onEdit(scope) {
            ...
            this.title = '编辑项目';
            this.dialogObj.bproject_name = scope.row.bproject_name;
            this.dialogObj.bporject_desc = scope.row.bporject_desc;
            this.dialogObj.remarks = scope.row.remarks;
            this.dialogObj.status = scope.row.status;
            this.dialogObj.creator = scope.row.creator;
            this.dialogObj.bid = scope.row.bid;
        ...
        onAdd() {
            this.title = '创建项目';
            this.dialogObj.bproject_name = '';
            this.dialogObj.bporject_desc = '';
        ...
        getData() {
            // 项目检索状态+列表
            this.params.bproject_name = this.bproject_name;
            this.$api
                .on_search_big_project(this.params)
                .then(res => {
                    if (res.results.code == 0) {
        ...
        onDel(scope) {
            ...
            this.$confirm('此操作将永久删除该项目, 是否继续?', '提示', {
                ...
                .then(() => {
                    ...
                    this.$api.delete_big_project(params).then(res => {
                        if (res.code == 0) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
