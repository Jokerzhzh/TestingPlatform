<template>
    <div>
        <div class="container">
            ...
            <el-table :data="tableData" v-loading="loading" border size="mini" class="table" ref="multipleTable">
                ...
                <el-table-column prop="rname" label="需求名称" width="200">
                    <!-- <template slot-scope="scope">
                        <span v-for="(item, index) in scope.row.requid" :key="index">
                            {{ item.rname }}
                        </span>
                    </template>-->
                </el-table-column>
                <el-table-column prop="sum_r" label="用例个数" width="200"></el-table-column>
                <el-table-column prop="pass_case_sum" label="通过用例个数" width="200"></el-table-column>
                <el-table-column prop="bug_sum" label="BUG数量" width="200"></el-table-column>
                ...
                <!-- <el-table-column label="bug缺陷" width="350">
                    <template slot-scope="scope">
                        <el-tag
                            ...
                            v-for="(item, index) in scope.row.bug_info.bug_key"
                ...
                <el-table-column label="操作" width="200" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="primary" @click="onEdit(scope.row)">查看详情</el-button>
        ...
        <el-dialog title="组合用例" :visible.sync="dialogGroup" width="80%" :before-close="handleClose" :close-on-click-modal="false">
            ...
            >
                ...
                <el-table-column label="状态">
                    <template slot-scope="scope">{{ scope.row.status == true ? '可用' : '不可用' }}</template>
                </el-table-column>
                <el-table-column label="备注" width="200">
                    <template slot-scope="scope">{{ scope.row.note }}</template>
                </el-table-column>
        ...
        <!-- 用例执行弹窗 -->
        <el-dialog title="编辑用例执行" :visible.sync="dialogVisible" width="80%" :before-close="handleClose">
            <div class="public-dialog">
        ...
        <!-- bug 弹窗 -->
        <el-dialog title="新建bug" :visible.sync="bugVisible" width="80%" :before-close="closeBug">
            <div class="public-dialog">
                <el-card class="box-card">
                ...
                <el-table-column label="预期结果">
                  ...
                  <el-table-column label="执行状态">
                    ...
                    <a href="javascript:;" class="file" style="margin-top: 10px">
                        上传附件
                        <input type="file" name id="fileField" multiple @change="getvalueName()" />
                    </a>
                    <div style="line-height: 40px; margin-left: 20px; font-weight: bolder" v-for="(item, index) in files" :key="index">
                        {{ item.name }}
                    </div>
                </el-card>
                <el-card class="box-input-card">
                    <el-form status-icon label-width="100px" class="demo-ruleForm">
                        ...
                        <el-form-item label="项目" :rules="[{ required: true, message: '请选择项目', trigger: 'blur' }]">
                            <el-select placeholder="请选择项目" v-model="bugEdit.project_name">
                                <el-option
  ...
  <p>{{caseName}}</p>
                        ...
                        <el-form-item label="指派" prop="checkPass">
                            <el-select v-model="bugEdit.assignee" filterable placeholder="请选择指派人选" class="public-form">
                                <el-option
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="onSubmitBug">提 交</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
            ...
            this.$api
                ...
                .then((res) => {
                    if (res.code == 0) {
                        ...
                        console.log('tableGroupData', this.tableGroupData);
        ...
        onSubmitBug() {
            ...
            formData.append('issuetype', this.bugEdit.issuetype);
        ...
        onDel(data) {
            ...
            this.$confirm('此操作将永久删除该类目, 是否继续?', '提示', {
                ...
                .then(() => {
                    let formData = new FormData();
                    formData.append('requid', data.requid);
        ...
        // 编辑
        onEdit(data) {
            //    rname  path: '/testUsecaseDetails',
            this.$router.push({ path: '/testUsecaseDetails', query: { rqname: data.rname } });
        },
        getData() {
            // 项目检索状态+列表
            var params = {
